...
Observation files are generated once and hashed, then replayed to all compared systems. Generation fingerprints include configuration, scoring, pinned package versions, and generator/evaluator code hashes. Timings are measured separately and are not expected to reproduce exactly. Tuning and evaluation use disjoint seed ranges. The included baseline's fixed settings were not tuned against held-out results.
